...
In this paper, a Thorlabs C240TME-C aspheric lens with NA = 0.5 is used for the demonstration. As the $1/e^2$ radius of the input beam is 5.2-mm, the overfilled factor regards to the entrance pupil ($\Phi = 8 mm$) is 1.3. Therefore, when the apodization factor $G$ (refers to the rate of decrease of the beam amplitude as a function of radial pupil coordinate) is set as $\sqrt{1/1.3}$, i.e. 0.877. And the beam amplitude is normalized to unity at the center of the pupil. The amplitude at other points in the entrance pupil is given by $A(\rho)=e^{-G\rho^2}$, where $\rho$ is the normalized pupil coordinate. Under this approximation, the aberration induced by the lens is calculated and represented by the standard zernike coefficients. In Tab. \ref{tab:A1} the nonzero coefficients are listed upto term 37. For the simplicity, we first dedicate to the in-air focus condition. Therefore, the refractive index $n_2$ of the medium after the lens is 1. And the normalized intensity distribution near the focus can be calculated based on \eqref{eq:17} and \eqref{eq:23}. 
